# TarFlow GRF-256 Field-Level Example Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** A field-level example training a conditional TarFlow p(field | theta) on the 256×256 `gaussian_random_field_256` task — the harder sibling of the implemented 32×32 example — with RoPE positional encoding, vector-prefix theta conditioning, 50k training steps, and the same diagnostics plus an exact held-out NLL.

**Architecture:** One standalone script `train_tarflow_grf256.py` (yaml-driven, import-safe) built on `TarFlow(modeled="image", use_rope=True, cond="vector")` and `ConditionalFlowPipeline(structured_obs=True)`; patch 16 → 256 tokens, channels 512 (53.6M params). Diagnostics helpers are copied verbatim from the FM script in the same benchmark dir (importing it would force `JAX_PLATFORMS=cpu`). A pytest file mirrors `tests/test_train_tarflow_grf.py`.

**Tech Stack:** JAX / flax.nnx, GenSBI (editable checkout at `/lustre/ific.uv.es/ml/ific088/github/GenSBI`), sbibm_jax datasets, pytest.

**Spec:** `docs/superpowers/specs/2026-07-11-tarflow-grf256-design.md`
**Reference implementation:** `examples/sbi-benchmarks/gaussian_random_field/tarflow/train_tarflow_grf.py` (the finished 32×32 example — this plan's code is that script with 256 substitutions already applied; do not re-derive from it)

## Global Constraints

- Python for all commands: `/lhome/ific/a/aamerio/miniforge3/envs/gensbi/bin/python` (the `gensbi` conda env).
- All tests and smoke runs on CPU: the script sets `JAX_PLATFORMS=cpu` when imported; tests set it defensively too.
- No GenSBI or sbibm_jax library changes.
...
- Diagnostics helpers are copied verbatim from `examples/sbi-benchmarks/gaussian_random_field_256/train-grf.py` (the FM script in the same dir).
- Datasets stay float32: do NOT pass a dtype argument to OnlineTaskDataset/TaskDataset (deviation from the FM 256 script's bfloat16 — the exact-likelihood NLL wants float32).
- `to_obs_cond` returns `(x, theta)` unchanged — sbibm_jax's collate already tokenizes theta to (B, 2, 1). Do NOT add `theta[..., None]` (that bug crashed the 32×32 smoke run and was fixed there).
- Production config trains 50000 steps (user requirement).
...
**Files:**
- Create: `examples/sbi-benchmarks/gaussian_random_field_256/tarflow/train_tarflow_grf256.py`
- Create: `examples/sbi-benchmarks/gaussian_random_field_256/tarflow/config/config_1.yaml`
- Test: `tests/test_train_tarflow_grf256.py`
...
Create `tests/test_train_tarflow_grf256.py`:
...
/lhome/ific/a/aamerio/miniforge3/envs/gensbi/bin/python -m pytest tests/test_train_tarflow_grf256.py -v
```
...
```yaml
# GRF-256 TarFlow field-level NLE — experiment 1: rope + vector-prefix cond
# 53.6M params; ~40 GB A100. Halve batch_size on OOM.
model:
  img_size: 256
  patch_size: 16           # -> T = 256 tokens, F = 256 per token
  img_channels: 1
  cond_dim: 2              # theta = (log_std, alpha) -> 2 prefix tokens
  use_rope: true           # 2D rotary PE; learned pos_embed dropped
  rope_theta: 10000
  head_dim: 64             # % 4 == 0 required by rope; >= 32 recommended
  num_heads: 8             # channels = 512
  num_blocks: 8
  layers_per_block: 2
  permutation: flip
...
training:
  batch_size: 64           # 256x256 fields: FM PixelDiT precedent at this res
  val_batch_size: 64
  max_workers: 4           # online sim workers (grain prefetch if offline)
  online: true             # fresh prior+simulator draws every batch
  nsteps: 50000            # harder problem -> 5x the 32-case budget
  ema_decay: 0.999
  val_every: 100
  early_stopping: false    # 50k steps IS the budget for this baseline
  multistep: 1
  experiment_id: 1
  train_model: true
  restore_model: false
...
```bash
git add examples/sbi-benchmarks/gaussian_random_field_256/tarflow tests/test_train_tarflow_grf256.py
git commit -m "feat(grf256 tarflow): script skeleton — rope + vector-prefix flow builder, config"
```
...
**Files:**
- Modify: `examples/sbi-benchmarks/gaussian_random_field_256/tarflow/train_tarflow_grf256.py` (append after `build_training_config`)
- Test: `tests/test_train_tarflow_grf256.py` (append)
...
Append to `tests/test_train_tarflow_grf256.py`:
...
Run:
```bash
/lhome/ific/a/aamerio/miniforge3/envs/gensbi/bin/python -m pytest tests/test_train_tarflow_grf256.py -v -k "to_obs_cond or bits_per_dim"
```
Expected: 2 FAILED with `AttributeError: module 'train_tarflow_grf256' has no attribute ...`.

- [ ] **Step 3: Implement the helpers**

Append to `train_tarflow_grf256.py`:
...
```bash
git add examples/sbi-benchmarks/gaussian_random_field_256/tarflow/train_tarflow_grf256.py tests/test_train_tarflow_grf256.py
git commit -m "feat(grf256 tarflow): batch map + exact held-out bits/dim NLL"
```

---

### Task 3: Diagnostics helpers (copied from the FM script in the same dir)

**Files:**
- Modify: `examples/sbi-benchmarks/gaussian_random_field_256/tarflow/train_tarflow_grf256.py` (append after `heldout_bits_per_dim`)
- Test: `tests/test_train_tarflow_grf256.py` (append)
...
Run:
```bash
/lhome/ific/a/aamerio/miniforge3/envs/gensbi/bin/python -m pytest tests/test_train_tarflow_grf256.py -v -k "radial or plot_helpers"
```
...
`train_tarflow_grf256.py` (after `heldout_bits_per_dim`), in this order:
`radial_power_spectrum`, `plot_losses`, `plot_field_grid`,
`theory_power_spectrum`, `plot_power_spectra`.
...
```bash
git add examples/sbi-benchmarks/gaussian_random_field_256/tarflow/train_tarflow_grf256.py tests/test_train_tarflow_grf256.py
git commit -m "feat(grf256 tarflow): P(k) + plotting diagnostics copied from FM script"
```
...
**Files:**
- Modify: `examples/sbi-benchmarks/gaussian_random_field_256/tarflow/train_tarflow_grf256.py` (append `main` + `__main__` block at end of file)
- Create: `examples/sbi-benchmarks/gaussian_random_field_256/tarflow/config/config_smoke.yaml`
- Test: `tests/test_train_tarflow_grf256.py` (append)

**Interfaces:**
- Consumes: everything from Tasks 1–3 (`build_flow`, `build_training_config`, `to_obs_cond`, `heldout_bits_per_dim`, the five diagnostics helpers); `ConditionalFlowPipeline(flow, train, val, dim_obs, dim_cond, structured_obs=True, training_config=...)`; `pipeline.train(rngs, save_model=True) -> (loss_array, val_loss_array)`; `pipeline.sample(key, x_o, nsamples, use_ema) -> (nsamples, 256, 256, 1)`; `pipeline.restore_model()`; `OnlineTaskDataset` / `TaskDataset` from sbibm_jax.
...
Run:
```bash
/lhome/ific/a/aamerio/miniforge3/envs/gensbi/bin/python -m pytest tests/test_train_tarflow_grf256.py -v -k smoke
```
...
# Not a science run. Requires the gaussian_random_field_256 HF dataset cache
# (val loader + df_test come from the offline split).
model:
  img_size: 256
  patch_size: 32           # -> T = 64 tokens (fast CPU sampling)
  img_channels: 1
  cond_dim: 2
  use_rope: true
  rope_theta: 10000
...
Run (needs the `gaussian_random_field_256` HF cache, present on this cluster —
the FM example trained from it). This is a 256×256 CPU run: expect it to take
noticeably longer than the 32×32 smoke (roughly 10–30 minutes); let it finish.
```bash
cd examples/sbi-benchmarks/gaussian_random_field_256/tarflow
JAX_PLATFORMS=cpu /lhome/ific/a/aamerio/miniforge3/envs/gensbi/bin/python \
    train_tarflow_grf256.py --config config/config_smoke.yaml
cd -
```
Expected output: parameter count print, 20 training steps, two
`held-out NLL [...] bits/dim` lines (finite values), `[ema] theta 0: sampled
(2, 256, 256), finite=True` and the `[raw]` twin, and five PNGs under
`examples/sbi-benchmarks/gaussian_random_field_256/tarflow/imgs/`
(`grf256_loss_conf99.png`, `grf256_fields_conf99_{ema,raw}.png`,
`grf256_pk_conf99_{ema,raw}.png`). Sample quality is irrelevant at 20 steps —
only that every stage runs and stays finite.

If the smoke run fails inside pipeline/dataset code, fix the script's usage
(shapes, argument names), not the library. Re-run until it completes. After
success, delete the experiment-99 checkpoint dirs
(`checkpoints/99`, `checkpoints/ema/99`) and leave the smoke PNGs untracked.

- [ ] **Step 7: Commit**

```bash
git add examples/sbi-benchmarks/gaussian_random_field_256/tarflow/train_tarflow_grf256.py examples/sbi-benchmarks/gaussian_random_field_256/tarflow/config/config_smoke.yaml tests/test_train_tarflow_grf256.py
git commit -m "feat(grf256 tarflow): main() — train, held-out NLL, sample, P(k) diagnostics"
```
...
**Files:**
- Create: `sub/train_model_tarflow_grf256.sub`
- Reference (pattern source): `sub/train_model_tarflow_grf.sub`

**Interfaces:**
- Consumes: the finished `train_tarflow_grf256.py` + `config/config_1.yaml` (Tasks 1–4).
- Produces: cluster submission entry point (submitted by the controller session after all tasks complete, per user request — the implementer must NOT run condor_submit).

- [ ] **Step 1: Create the submit file**

Create `sub/train_model_tarflow_grf256.sub` (mirrors `sub/train_model_tarflow_grf.sub`; note the `workdir` uses the `/lhome/.../data/github` path alias like every other sub file):

```
experiment_name = tarflow_grf256
version = 1a
workdir = /lhome/ific/a/aamerio/data/github/GenSBI-examples/examples/sbi-benchmarks/gaussian_random_field_256/tarflow
script_path = train_tarflow_grf256.py
...
```bash
git add sub/train_model_tarflow_grf256.sub
git commit -m "feat(grf256 tarflow): condor submit file"
```

- [ ] **Step 4: Report submission handoff**

The GPU science run (50k steps, ~5–9 h estimated) is submitted by the
controller session after all tasks pass review — NOT by the implementer of
this task. Note in the task report that the sub file is ready and untested
