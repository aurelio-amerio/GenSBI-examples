...
def main():
    ...
    params_flux = Flux1Params(
        ...
        id_embedding_kind=("absolute", "pos1d"),
        rngs=nnx.Rngs(0),
        param_dtype=jnp.bfloat16,
    )
    ...
    training_config["checkpoint_dir"] = (
        "/home/zaldivar/symlinks/aure/Github/GenSBI-examples/tests/gw_npe_v6b/checkpoints"
    )
    ...
    samples = pipeline_latent.sample_batched(
        nnx.Rngs(0).sample(), x_o, 100_000, chunk_size=10_000, encoder_key=jax.random.PRNGKey(1234)
    )
    ...
    plt.savefig(f"gw_samples_v6b_conf{experiment}.png", dpi=100, bbox_inches="tight")
    plt.show()
    ...
    plot_tarp(ecp, alpha)
    plt.savefig(f"gw_tarp_v6b_conf{experiment}.png", dpi=100, bbox_inches="tight") # uncomment to save the figure
    plt.show()
