...
def main():
    ...
    dim_cond_latent = (
        vae_model.latent_shape[1] * 2
    )  # we have 2 latent vectors (one for each channel)
    ...
    params_flux = Flux1Params(
        ...
        id_embedding_strategy=("absolute", "pos1dd"),
        rngs=nnx.Rngs(0),
        param_dtype=jnp.bfloat16,
    )
    ...
    plot_tarp(ecp, alpha)
    plt.savefig(
        f"gw_tarp_v7_conf{experiment}.png", dpi=100, bbox_inches="tight"
