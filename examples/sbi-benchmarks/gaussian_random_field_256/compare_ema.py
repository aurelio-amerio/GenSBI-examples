"""Compare a trained GRF-256 checkpoint's samples WITH vs WITHOUT EMA weights.
...
Usage (conda env ``gensbi``):
    python compare_ema.py --config config/config_7.yaml
    python compare_ema.py --config config/config_7.yaml --nsamples 8 --step_size 0.02
    python compare_ema.py --config config/config_7.yaml --device cpu
"""
...
# JAX_PLATFORMS; default to cuda for the 256x256 model; --device overrides.
...
TASK_NAME = "gaussian_random_field_256"  # 256x256 field
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="config/config_7.yaml")
