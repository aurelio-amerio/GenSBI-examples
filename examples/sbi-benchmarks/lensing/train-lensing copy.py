...
# xs_mean = np.mean(df_train["xs"], axis=(0, 1, 2), keepdims=True)
# thetas_mean = np.mean(df_train["thetas"], axis=0, keepdims=True)

# xs_std = np.std(df_train["xs"], axis=(0, 1, 2), keepdims=True)
...
ch_latent = vae_model.latent_shape[3] * 4

# %%
dim_cond_latent, ch_latent
# %%

params_dict_flux = parse_flux1_params(config_path)
# %%
params_dict_flux["context_in_dim"] = ch_latent
# %%
...
train_dataset_npe = (
    train_dataset_npe.batch(batch_size).map(split_data)
...
# %%
obs, cond = next(iter(train_dataset_npe))
# %%
vae_model.latent_shape
...
pipeline_latent = ConditionalFlowPipeline(
    ...
    val_dataset_npe,
    dim_obs=dim_obs,
    dim_cond=(latent_dim1, latent_dim2),  # we are workin in the latent space of the vae
    ch_obs=ch_obs,
    ch_cond=ch_latent,  # conditioning is now in the latent space
    training_config=training_config,
    id_embedding_strategy=("absolute", "rope2d"),
)
# %%
t = jnp.array([0.0], dtype=jnp.bfloat16)
obs, cond = next(iter(train_dataset_npe))
...
cond_ids.shape
# %%
pipeline_latent.model(t=t, obs=obs, cond=cond, obs_ids=obs_ids, cond_ids=cond_ids)

# %%

pipeline_latent.train(nnx.Rngs(0), 2, save_model=True)
