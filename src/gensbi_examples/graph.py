...
def find_ancestors_jax(mask, node):
    ...
    def body_fn(carry, i):
        ...
        def process_node_fn(is_ancestor, stack, write_idx):
        ...
        def skip_node_fn(is_ancestor, stack, write_idx):
            return is_ancestor, stack, write_idx

        # Only process if i is within the valid bounds of the stack
        is_valid_node = i < write_idx
        is_ancestor, stack, write_idx = jax.lax.cond(is_valid_node, process_node_fn, skip_node_fn, is_ancestor, stack, write_idx)
        return (is_ancestor, stack, write_idx), None
