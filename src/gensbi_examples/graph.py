...
def find_ancestors_jax(mask, node):
    ...
    def body_fn(carry, i):
        ...
        def inner_body_fn(carry, j):
            ...
            cond = (value != 0) & (j != current_node) & (~is_ancestor[j]) & is_valid_node
            
            def true_fn(carry_inner):
                is_ancestor, stack, write_idx = carry_inner
        ...
        carry, _ = jax.lax.scan(inner_body_fn, (is_ancestor, stack, write_idx), jnp.arange(num_nodes))
        return carry, None
