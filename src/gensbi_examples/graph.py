...
def find_ancestors_jax(mask, node):
    ...
    def body_fn(carry, i):
        ...
        def process_node(carry_inner):
            ...
            (is_ancestor_inner, stack_inner, write_idx_inner), _ = jax.lax.scan(inner_body_fn, (is_ancestor_inner, stack_inner, write_idx_inner), jnp.arange(num_nodes))
            return is_ancestor_inner, stack_inner, write_idx_inner

        def skip_node(carry_inner):
            return carry_inner
            
        is_valid_node = i < write_idx
        is_ancestor, stack, write_idx = jax.lax.cond(is_valid_node, process_node, skip_node, (is_ancestor, stack, write_idx))
