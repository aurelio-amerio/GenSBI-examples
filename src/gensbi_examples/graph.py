...
def find_ancestors_jax(mask, node):
    ...
    def body_fn(carry, i):
        is_ancestor, stack, head, tail = carry

        has_elements = head < tail

        def process_node(val):
            is_ancestor, stack, head, tail = val
            current_node = stack[head]
            head = head + 1
            current_parents = mask[current_node, :]

            def inner_body_fn(inner_carry, j):
                is_ancestor, stack, tail = inner_carry
                value = current_parents[j]
                cond = (value != 0) & (j != current_node) & (~is_ancestor[j])

                def true_fn(val):
                    is_ancestor, stack, tail = val
                    is_ancestor = is_ancestor.at[j].set(True)
                    stack = stack.at[tail].set(j)
                    tail = tail + 1
                    return is_ancestor, stack, tail

                is_ancestor, stack, tail = jax.lax.cond(
                    cond, true_fn, lambda x: x, (is_ancestor, stack, tail)
                )
                return (is_ancestor, stack, tail), None

            (is_ancestor, stack, tail), _ = jax.lax.scan(
                inner_body_fn, (is_ancestor, stack, tail), jnp.arange(num_nodes)
            )
            return is_ancestor, stack, head, tail
        ...
        return (
            jax.lax.cond(
                ...
                process_node,
                lambda x: x,
                (is_ancestor, stack, head, tail),
            ),
            None,
        )

    (is_ancestor, stack, head, tail), _ = jax.lax.scan(
        body_fn, (is_ancestor, stack, head, tail), jnp.arange(num_nodes)
    )
