...
checkpoint_dir_ema = f"{notebook_path}/checkpoints/{task_name}_simformer/ema"
...
training_config = SimformerPipeline._get_default_training_config()
...
pipeline = SimformerPipeline(
    train_dataset, val_dataset, dim_theta.item(), dim_data.item(), params, training_config
)
...
def get_samples(idx, nsamples=10_000, use_ema=False):
    observation, reference_samples = task.get_reference(idx)
    true_param = jnp.array(task.get_true_parameters(idx))

    rng = jax.random.PRNGKey(45)

    samples = pipeline.sample(rng, observation, nsamples, step_size=0.01, use_ema=use_ema)
...
for idx in range(1, 11):
    samples, true_param, reference_samples = get_samples(idx, nsamples=10_000, use_ema=False)
